...
namespace Server.Controllers
{
    ...
    public class UserController : Base_Control_Api
    {
        ...
        public UserController(IUnitOfWork unitOfWork, IMapper mapper, UserManager<User_data> userManager, IEmailSender emailSender)
        {
        ...
        {
            var user = await _unitOfWork.UserRepository.GetFirstOrDefaultAsync(x => x.UserName == userName);
            if (user == null) return NotFound("User not found");
        ...
        public async Task<ActionResult<UserModelDto>> upsertUser([FromBody] UserModelDto upsertDto)
        {
            try
            {
                ...
                else
                {
                    ...
                    {
                        To = newUser.Email,
                        Subject = "Welcome to Abbsium - Your account has been created",
                        Body = NewUserTemplate.Build(newUser.UserName, defaultPassword)
                    });
        ...
        public async Task<IActionResult> AdminResetPassword([FromBody] AdminResetPasswordDto dto)
        {
            ...
            {
                To = user.Email,
                Subject = "Your Abbsium password has been reset",
                Body = AdminPasswordResetTemplate.Build(dto.NewPassword)
            });

            return Ok(new AuthResponseDTO
            {
                Success = true,
                Message = "Password has been reset successfully and notification email sent"
            });
        }
